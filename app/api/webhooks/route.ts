...
export async function POST(req: Request) {
    ...
    if (data.subscription.status === "webhook_callback_verification_pending") {
        ...
        if (await verifyMessage(req, msg)) {
            console.log("Verification runtime:", performance.now() - startTime);
            const giveaway = await prisma.giveaways.findFirst({ where: { twitchId: data.event.reward.id }, select: { id: true, creatorId: true } });
            console.log("Giveaway fetch runtime:", performance.now() - startTime);
            if (giveaway) {
                ...
                console.log("Viewer upsert runtime:", performance.now() - startTime);
                ...
                console.log("ViewerOnStream upsert runtime:", performance.now() - startTime);
                ...
                console.log("Redemption create runtime:", performance.now() - startTime);
