...
export default async function WheelPage({ params }: Props) {
    ...
    const thisUser = await prisma.account.findFirst({
        where: { providerAccountId: session.user.providerAccountId },
        select: { userId: true },
    });
    if (giveaway && thisUser) {
        ...
        if (thisUser.userId === giveaway.creatorId) {
            ...
            const isMod = await prisma.moderator.findFirst({
                where: { moderatorId: thisUser.userId, creatorId: giveaway.creatorId },
                select: { id: true },
            });
            if (isMod) {
                return (
